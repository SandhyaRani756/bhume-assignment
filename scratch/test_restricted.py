...
def main():
    ...
    # Define restricted search grid
    g_rx = int(round(mdx))
    g_ry = int(round(mdy))
    
    # Generate offsets
    offsets = set()
    # 1. Neighborhood around (0,0)
    for dx in range(-8, 9):
        for dy in range(-8, 9):
            offsets.add((dx, dy))
    # 2. Neighborhood around (g_rx, g_ry)
    for dx in range(g_rx - 8, g_rx + 9):
        for dy in range(g_ry - 8, g_ry + 9):
            offsets.add((dx, dy))
            
    grid = np.array(sorted(list(offsets)), dtype=float)
    print(f"Restricted grid size: {len(grid)} offsets")
    ...
    for pn in truths_3857.index:
        ...
        print(f"Plot {pn}: Best translation dx={best_dx:.2f}m, dy={best_dy:.2f}m | dist: {best_dist:.2f}")
    ...
    preds_local['method_note'] = 'restricted grid local search'
    ...
    print("\n--- Restricted Grid Locally Aligned Score ---")
    print(score_local)
